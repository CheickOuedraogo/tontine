...
const startTontine = asyncHandler(async (req, res) => {
  ...
  const dateDebut = new Date((req.body && req.body.dateDebut) || Date.now());
...
// GET /api/tontines/open — public tontines available to join
const getOpenTontines = asyncHandler(async (req, res) => {
  const tontines = await tontineQ.findPublicOpen();
  // Filter out tontines that are already full
  const available = tontines.filter(t => t.nbMembresActuel < t.nbMembresAttendu);
  // Check membership for current user
  const withMembership = await Promise.all(available.map(async (t) => {
    const isMember = await tontineQ.isMember(req.user.id, t.id);
    return { ...t, estMembre: isMember };
  }));
  res.json({ success: true, tontines: withMembership });
});

// POST /api/tontines/:tontineId/join — join a tontine
const joinTontine = asyncHandler(async (req, res) => {
  const { tontineId } = req.params;
  const tontine = await tontineQ.findById(tontineId);
  if (!tontine) throw new ApiError(404, 'Tontine introuvable');
  if (tontine.statut !== 'EN_ATTENTE') throw new ApiError(400, 'Cette tontine n\'accepte plus de nouveaux membres');

  const already = await tontineQ.isMember(req.user.id, tontineId);
  if (already) throw new ApiError(400, 'Vous etes deja membre de cette tontine');

  const membres = await tontineQ.findMembres(tontineId);
  if (membres.length >= tontine.nbMembresAttendu) throw new ApiError(400, 'Cette tontine est complete');

  // Check contract exists
  const contrat = await contratQ.findByTontine(tontineId);
  if (!contrat) throw new ApiError(400, 'Aucun contrat n\'a ete genere pour cette tontine. Contactez le createur.');
  ...
  try {
    ...
    await contratQ.signer({ contratId: contrat.id, userId: req.user.id, ipAddress: ip });
  } catch (signErr) {
    console.error('Erreur signature auto:', signErr.message);
  }
  ...
  await notifQ.create({
    ...
    contenu: `Un nouveau membre a rejoint et signe le contrat de "${tontine.nom}"`,
    lienAction: `/tontines/${tontineId}`
  });

  res.json({ success: true, message: 'Vous avez rejoint la tontine et signe le contrat' });
});

module.exports = { createTontine, getMesTontines, getTontine, getMembres, startTontine, getOpenTontines, joinTontine };
